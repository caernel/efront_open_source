...
/api2.php?token=<token>&action=users                                                		returns all users defined in eFront
/api2.php?token=<token>&action=user_info&user=<user_login>                                  returns <user_login> information
...
/api2.php?token=<token>&action=user_groups&login=<login> 								returns the groups that are assigned to the user <login>
...
/api2.php?token=<token>&action=categories
    ...
    if ($GLOBALS['configuration']['api'] && eF_checkIP('api')) {    	
        if (isset($_GET['action'])) {
            switch($_GET['action']) {
                    ...
                    if (isset($_GET['token']) && checkToken($_GET['token'])) {
                        if (isset($_GET['login'])) {
							...
							try {
								...
								$userProfile = eF_getTableData("user_profile", "*", "active=1");
								foreach($userProfile as $value){
									$user -> user[$value['name']] = $_GET[$value['name']];
								}
								$user -> persist();
                ...
                case 'users':{
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		$users = eF_getTableData("users", "id,login,user_type,email,active,name,surname", "archive=0");
                		echo "<xml>";
                		echo "<users>";
                		foreach ($users as $key => $user){
                			...
                			echo "<user_types>".$user['user_types']."</user_types>";
                			echo "</user>";
                		}
                		echo "</users>";
                		echo "</xml>";
                        ...
                        if (isset($_GET['lesson'])) {
                            ...
                            try {
								...
								foreach ($content as $item) {
									$unit = new EfrontUnit($item);
									$unit = $unit -> getArrayCopy();
									echo "<unit>";
									echo "<id>".$unit['id']."</id>";
									echo "<name>".$unit['name']."</name>";
									echo "<parent_id>".$unit['parent_content_ID']."</parent_id>";
									echo "<type>".$unit['ctg_type']."</type>";
									echo "</unit>";
								}
								echo "</units>";
								echo "<metadata>";
                		...
                		if (isset($_GET['login'])) {
                			...
                			try {
                				...
                				foreach ($groupsList as $key => $group) {
                					...
                					echo "<active>".$group['active']."</active>";
                					echo "</group>";
                    ...
                    if (isset($_GET['token']) && checkToken($_GET['token'])) {
                        if (isset($_GET['login'])) {
                            try {
                                ...
                                echo "<first_name>".$user -> user['name']."</first_name>";
                                echo "<last_name>".$user -> user['surname']."</last_name>";
                                echo "<active>".$user -> user['active']."</active>";
                                echo "<user_type>".$user -> user['user_type']."</user_type>";
                                echo "<email>".$user->user['email']."</email>";
                                echo "</general_info>";
                                echo "</xml>";
                            } catch (Exception $e) {
								if ($e -> getCode() == EfrontUserException :: USER_NOT_EXISTS) {
                ...
                case 'categories':{
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		$result = eF_getTableData("directions", "id, name, parent_direction_ID", "active=1");
                		echo "<xml>";
                
                		echo "<categories>";
                		foreach($result as $direction){
                			$result_courses = eF_getTableData("courses", "id, name, price", "active=1 AND show_catalog=1 AND directions_ID=".$direction['id']);
                			$courses = array();
                			foreach($result_courses as $course) {
                				$course['type'] = 'course';
                				$courses[] = $course;
                			}
                			$result_lessons = eF_getTableData("lessons", "id, name, price", "active=1 AND show_catalog=1 AND directions_ID=".$direction['id']);
                			$lessons = array();
                			foreach($result_lessons as $lesson) {
                				$lesson['type'] = 'lesson';
                				$lessons[] = $lesson;
                			}
                			$items = array_merge($courses, $lessons);
                			$items = eF_multiSort($items,"name");
                ...
                case 'category':{
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		if (isset($_GET['category'])){
                			...
                			$result = eF_getTableData("directions", "id, name, parent_direction_ID", "active=1 and id=".$_GET['category']);
                			if(!empty($result)){
                				$direction = $result[0];
                				$result_courses = eF_getTableData("courses", "id, name, description, price", "active=1 AND show_catalog=1 AND directions_ID=".$direction['id']);
                				$courses = array();
                				foreach($result_courses as $course) {
                					$course['type'] = 'course';
                					$courses[] = $course;
                				...
                				echo "<xml>";
                				echo "<status>error</status>";
                				echo "<message>Category does not exit</message>";
                				echo "</xml>";
                			}
                		} else {
                			echo "<xml>";
                ...
                case 'buy_course':{
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		...
                		$paypal_variables = array(
                				"notify_url" => G_SERVERNAME.'ipn.php',
                				"cmd" => "_xclick",
                				"bn" => "efront_BuyNow_WPS_GR",
                				"amount" => $course -> course['price'],
                				"item_name" => $course -> course['name'],
                				"custom" => $user->user['login'],
                				"item_number" => ":".$course -> course['id']."::",
                				"charset" => "utf-8",
                				"return" => G_SERVERNAME . 'index.php?ctg=checkout&checkout=1&return_paypal=1',
                				"rm" => "2",
                				"cancel_return" => G_SERVERNAME . 'index.php?ctg=checkout&checkout=1&register_lessons=1&message=You+have+cancelled+the+transaction',
                				"currency_code" => $GLOBALS['configuration']['currency'],
                				"business" => strtolower($ef_conf['paypalbusiness']),
                				...
                				"no_note" => "1",
                				"no_shipping" => "1",
                				"cbt" => "Return to ".$ef_conf['site_name'],
                		);
                ...
                case 'buy_lesson':{ 
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		...
                		$paypal_variables = array(
                				...
                				"item_name" => $lesson -> lesson['name'],
                				"custom" => $user->user['login'],
                				"item_number" => $lesson -> lesson['id'].":::",
                				"charset" => "utf-8",
                				"return" => G_SERVERNAME . 'index.php?ctg=checkout&checkout=1&return_paypal=1',
                				"rm" => "2",
                				"cancel_return" => G_SERVERNAME . 'index.php?ctg=checkout&checkout=1&register_lessons=1&message=You+have+cancelled+the+transaction',
                	...
                	if (isset($_GET['token']) && checkToken($_GET['token'])) {
                		try {
							...
							if ($user -> user['autologin'] == "" ) {
								...
								echo "<xml>";
								echo "<status>error</status>";
								echo "<message>Autologin key already set</message>";
								echo "</xml>";
							}
                		} catch (Exception $e) {
                			echo "<xml>";
