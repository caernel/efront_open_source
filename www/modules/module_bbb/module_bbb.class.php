...
class module_BBB extends EfrontModule {
	...
	public function onInstall() {
		// Create module_BBB table - contains information about BBB conferences
		...
		// Create module_BBB_users_to_meeting table - contains information about users to BBB conferences
		eF_executeNew("DROP TABLE IF EXISTS `module_BBB_users_to_meeting`");
		$b = eF_executeNew(
			"CREATE TABLE `module_BBB_users_to_meeting` (
            	`users_LOGIN` varchar(255) NOT NULL,
				`meeting_ID` int(11) NOT NULL,
			KEY (`users_LOGIN`,`meeting_ID`)) DEFAULT CHARSET=utf8;");

		// Set default values for: BBB server(module_BBB_server), BBB salt(module_BBB_salt), BBB server version(module_BBB_server_version)	
	...
	public function onDeleteLesson($lessonId) {
		// When deleting a lesson delete all conferences concering this lesson
	...
	public function onExportLesson($lessonId) {
		// When exporting a lesson export all conferences concering this lesson
		$data = array();
	...
	public function onImportLesson($lessonId, $data) {
		// When importing a lesson import all conferences concering this lesson (see onExportLesson)
		$changed_ids = array();
	...
	public function getLessonCenterLinkInfo() {
		...
		if ($currentUser -> getRole($this -> getCurrentLesson()) == "professor") {
			return array('title' => _BBB, 'image' => $this -> moduleBaseDir . 'images/bbb32.png', 'link' => $this -> moduleBaseUrl);
		}
	}
	...
	public function getNavigationLinks() {
		...
		if ($currentUser -> getRole() == "administrator") {
			$basicNavArray = array( array('title' => _HOME, 'link' => "administrator.php?ctg=control_panel"), array('title' => _BBB, 'link' => $this -> moduleBaseUrl));
		} else {
			$basicNavArray = array( array('title' => _MYCOURSES, 'onclick' => "location='" . $currentUser -> getRole($this -> getCurrentLesson()) . ".php?ctg=lessons';top.sideframe.hideAllLessonSpecific();"), array('title' => $this -> getCurrentLesson() -> lesson['name'], 'link' => $currentUser -> getRole($this -> getCurrentLesson()) . ".php?ctg=control_panel"), array('title' => _BBB, 'link' => $this -> moduleBaseUrl));
	...
	public function getSidebarLinkInfo() {
		$link_of_menu_clesson = array( array('id' => 'BBB_link_id1', 'title' => _BBB, 'image' => $this -> moduleBaseDir . 'images/bbb16', 'eFrontExtensions' => '1', 'link' => $this -> moduleBaseUrl));
	...
	// Function to get the BBB server host
	...
	// Function to get the BBB server version
	...
	// Function to get the BBB security salt
	...
	private $BBB_logout_url = G_SERVERNAME;
	private function getBBBLogoutUrl(){
		if (!$this -> BBB_logout_url) {
			$BBB_logout_url = eF_getTableData("configuration", "value", "name = 'module_BBB_logout_url'");
			$this -> BBB_logout_url = $BBB_logout_url[0]['value'];
		}
		return $this -> BBB_logout_url;
	}
	...
	private function createBBBUrl($currentUser, $meeting_info, $always_joining = false) {
		...
		if ($currentUser -> getRole($this -> getCurrentLesson()) == "professor" && $meeting_info['status'] == 0 && $meeting_info . mayStart) {
			...
			if ($xml && $xml -> returncode == 'SUCCESS') {
				...
				$conferenceNameAndID = urlencode(utf8_decode($meeting_info['name']));
				$voiceBridge = 70000 + rand(0, 9999);
				$optionString = 'fullName=' . $fullName . '&meetingID=' . $conferenceNameAndID . '&password=' . $moderatorPassword . '&voiceBridge=' . $voiceBridge . '&welcome=' . urlencode(_WELCOMETO . ' %%CONFNAME%%')  . '&logoutURL=' . urlencode($this->getBBBLogoutUrl());
		...
		if (isset($_GET['postAjaxRequest'])) {
			...
			if ($_GET['insert'] == "true") {
				...
				foreach ($meeting_users as $user) {
					...
					$header = array('From' => $GLOBALS['configuration']['system_email'], 'To' => $user_mail, 'Subject' => $subject, 'Content-type' => 'text/plain;charset="UTF-8"', // if content-type is text/html, the message cannot be received by mail clients for Registration content
