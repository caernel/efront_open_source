...
{*					<tr><td class = "labelCell">{$T_COURSE_FORM.course_code.label}:&nbsp;</td>
			...
			{/if}
				{if $smarty.const.G_VERSIONTYPE != 'standard'} {* #cpp#ifndef STANDARD *}
					...
					<tr><td class = "labelCell">{$T_COURSE_FORM.branches_ID_autoselect.label}:&nbsp;</td>
						<td class = "elementCell">
							{$T_COURSE_FORM.branches_ID_autoselect.html}
							<img id = "busy" src = "images/16x16/clock.png" style = "display:none;" alt = "{$smarty.const._LOADING}" title = "{$smarty.const._LOADING}"/><div id = "autocomplete_branches" class = "autocomplete"></div>&nbsp;&nbsp;&nbsp;
						{if $T_MORE_LOCATIONS}
							<img src = "images/16x16/information.png" alt = "{$smarty.const._COURSEBRANCHES}" title = "{$smarty.const._COURSEBRANCHES}" class = "ajaxHandle" onclick = "eF_js_showHideDiv(this, 'course_branches_div', event)"/>
						{/if}
						</td></tr>
						{if $T_MORE_LOCATIONS}
					<tr><td></td>
						<td class = "infoCell">{$smarty.const._MORETHANONEBRANCHESONLYFIRSTSHOWING}</td></tr>
						{/if}
						{if $T_COURSE_FORM.supervisor_LOGIN}
					<tr><td class = "labelCell">{$T_COURSE_FORM.supervisor_LOGIN.label}:&nbsp;</td>
						<td class = "elementCell">{$T_COURSE_FORM.supervisor_LOGIN.html}</td></tr>
						{/if}
					{/if} {* #cpp#endif *}
...
{if ($smarty.const.G_VERSIONTYPE != 'community')} {* #cpp#ifndef COMMUNITY *}
	{if ($smarty.const.G_VERSIONTYPE != 'standard')} {* #cpp#ifndef STANDARD *}
					...
					{if !$T_COURSE_HAS_USERS}
						<select id = "lesson_mode_{$lesson.id}" name = "lesson_mode" onchange = "setLessonMode(this, '{$lesson.id}', this.options[this.options.selectedIndex].value)" {if !$lesson.has_lesson}style = "display:none"{/if}>
					...
					{else}
						{if $lesson.mode == "shared"}{$smarty.const._SHARED}{elseif $lesson.mode == "unique"}{$smarty.const._UNIQUE}{/if}
					{/if}
					</td>
	{/if} {* #cpp#endif *}
{/if} {* #cpp#endif *}
...
<!--ajax:usersTable-->
			<table style = "width:100%" class = "sortedTable" size = "{$T_TABLE_SIZE}" activeFilter = "1" sortBy = "6" order="desc" useAjax = "1" id = "usersTable" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" {if isset($T_JOBS_FILTER)}branchFilter="true" jobFilter="{$T_JOBS_FILTER}"{/if} url = "{$smarty.server.PHP_SELF}?ctg=courses&edit_course={$smarty.get.edit_course}&">
				<tr class = "topTitle">
			...
			{foreach name = 'users_to_lessons_list' key = 'login' item = 'user' from = $T_DATA_SOURCE}
				...
				{if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
						...
						<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=&op=evaluations">
		...
		{if !$T_SORTED_TABLE || $T_SORTED_TABLE == 'instancesTable'}
			<div class = "headerTools">
				<span>
					...
					<a href = "javascript:void(0)" onclick = "addInstance(Element.extend(this))" title = "{$smarty.const._ADDINSTANCE}" >{$smarty.const._ADDINSTANCE}</a>
				</span>
			</div>
<!--ajax:instancesTable-->
			...
			{if $_change_}
				<div class = "headerTools">
					...
					<span>
						...
						<a href = "javascript:void(0)" title = "{$smarty.const._IMPORTCOURSE}" onclick = "eF_js_showDivPopup(event, '{$smarty.const._IMPORTCOURSE}', 0, 'import_course_popup')">{$smarty.const._IMPORTCOURSE}</a></a>
					</span>
				</div>
