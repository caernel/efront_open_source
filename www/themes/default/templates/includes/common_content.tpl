			...
			<tr><td class = "moduleCell">
				{capture name = 't_insert_content_code'}
					...
					{/if}
							...
							{if !$T_SCORM}
								<tr style="display:none;" id = "ctg_type"><td class = "labelCell">{$T_ENTITY_FORM.ctg_type.label}:&nbsp;</td>
	...
	{elseif $smarty.get.bare}
		{if !$T_SCORM && $T_UNIT.ctg_type != 'tests' && $T_UNIT.ctg_type != 'feedback'}
	...
	{else}
		...
		{capture name = 't_content_footer_code'}
				...
				{/if}
					...
					{if $T_UNIT.options.complete_unit_setting == 0 && $T_UNIT.ctg_type != 'tests' && $T_UNIT.ctg_type != 'feedback' && !$_professor_}{assign var = "hideStyle" value = ''}{assign var = "show_content_footer" value = 1}{else}{assign var = "hideStyle" value = 'style = "visibility:hidden"'}{/if}
					{if $T_QUESTION}
						{assign var = "show_content_footer" value = 1}
		...
		{capture name = 't_content_code'}
				...
				{if $_change_ && !$_student_}
					...
					{if $T_UNIT}
						...
						{/if}
						{if !$T_SCORM_2004_TITLE && !$T_UNIT.linked_to}
						<span>
			...
			{if !$_student_ || !$T_RULE_CHECK_FAILED}
						...
						{/if}
			{/if}
			{if 'comments'|eF_template_isOptionVisible && $T_COMMENTS}
		...
		{capture name = 't_progress_bar'}
			{if $T_CURRENT_LESSON->options.tracking && $_change_ && $_student_}
				...
				{if $T_USER_PROGRESS.total_conditions > 0 && $T_CURRENT_LESSON->options.lesson_info}
					<a id = "lesson_passed" href = "{$smarty.server.PHP_SELF}?ctg=lesson_information&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._LESSONINFORMATION}', 2)" target = "POPUP_FRAME">
				...
				{/if}
				<div {if !$T_CURRENT_LESSON->options.timers || $T_SCORM || $T_UNIT.ctg_type == 'tests' || $T_UNIT.ctg_type == 'feedback'}style = "display:none"{/if}>{$smarty.const._TOTALTIMESPENTONTHISUNIT}:&nbsp;<span id = "user_time_in_unit_display"></span></div>
				<div {if !$T_CURRENT_LESSON->options.timers || $T_SCORM || $T_UNIT.ctg_type == 'tests' || $T_UNIT.ctg_type == 'feedback'}style = "display:none"{/if}>{$smarty.const._TOTALTIMESPENTONTHISLESSON}:&nbsp;<span id = "user_time_in_lesson_display"></span></div>
				<script>
				{if !$T_CURRENT_LESSON->options.timers || $T_SCORM || $T_UNIT.ctg_type == 'tests' || $T_UNIT.ctg_type == 'feedback'}var timers = false{else}var timers=true{/if} 
		...
		{capture name = 't_unit_operations'}
			...
			{if $_student_ && $T_CURRENT_LESSON->options.content_report}
				<div>{counter name = "unit_operations"}. <a href = "content_report.php?{$smarty.server.QUERY_STRING}" onclick = "eF_js_showDivPopup(event, '{$smarty.const._CONTENTREPORT}', 1)" target = "POPUP_FRAME">{$smarty.const._CONTENTREPORTTOPROFS}</a></div>
			{/if}
			{if $T_LESSON_FORUM}
				<div>{counter name = "unit_operations"}. <a href = "{$smarty.server.PHP_SELF}?ctg=forum&add=1&type=topic&forum_id={$T_LESSON_FORUM}&subject={$T_UNIT.name}", onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDFORUMPOSTONTHISUNIT}', 2)" target = "POPUP_FRAME" title="{$smarty.const._ADDFORUMPOSTONTHISUNIT}">{$smarty.const._ADDFORUMPOSTONTHISUNIT|eF_truncate:25:"...":true}</a></div>
			{/if}
			{if !$T_SCORM}
		...
		{capture name = "t_content_tools"}
			<div>{counter name = "content_tools"}. <a title = "{$smarty.const._UPLOADFILESANDIMAGES}" href = "{$smarty.server.PHP_SELF}?ctg=file_manager&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._UPLOADFILESANDIMAGES}', 3)" target = "POPUP_FRAME">{$smarty.const._UPLOADFILESANDIMAGES|eF_truncate:40}</a></div>
