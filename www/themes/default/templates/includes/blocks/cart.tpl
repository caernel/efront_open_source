...
{foreach name = 'cartlist' key = 'key' item = 'cartlist' from = $T_CART.lesson}
    ...
    <div class = "cartElement">
        ...
        <div class = "cartDelete">
            <span>{if $cartlist.price}{$cartlist.price_string}{elseif 'payments'|eF_template_isOptionVisible}{$smarty.const._FREEOFCHARGE}{/if}</span>
    ...
    {if $smarty.get.checkout && !$T_CART.credit}
   	<div class = "cartElement">
    	...
    	{if $T_CART.total_price != "" && $T_CART.total_price != 0}
    		...
    		<a id = "enter_coupon_link" href = "javascript:void(0)" onclick = "eF_js_showDivPopup(event, '{$smarty.const._COUPON}', 0, 'coupon_table')">{$smarty.const._CLICKTOENTERDISCOUNTCOUPON}</a>
			{/if} {* #cpp#endif *}
    	{/if}
   		</div>
   	&nbsp;</div>
