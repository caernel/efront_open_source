...
*}
				...
				{/if}
					...
					{if $smarty.const.G_VERSIONTYPE != 'standard'} {* #cpp#ifndef STANDARD *}
						...
						<tr><td class = "labelCell">{$T_LESSON_FORM.access_limit.label}:&nbsp;</td>
							<td class = "elementCell">{$T_LESSON_FORM.access_limit.html} {$smarty.const._TIMESAFTERREGISTRATION}</td></tr>
			...
			<table style = "width:100%" class = "sortedTable" size = "{$T_USERS_SIZE}" sortBy = "4" id = "usersTable" useAjax = "1" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" {if isset($T_JOBS_FILTER)}branchFilter="true" jobFilter="{$T_JOBS_FILTER}"{/if} url = "{$smarty.server.PHP_SELF}?ctg=lessons&edit_lesson={$smarty.get.edit_lesson}&">
				...
				{if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
						  <a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=evaluations&add_evaluation=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWEVALUATION}', 1)">
							  <img src="images/16x16/edit.png" title="{$smarty.const._NEWEVALUATION}" alt="{$smarty.const._NEWEVALUATION}"></a>
									...
									{capture name = 't_lesson_skills'}
											<div class = "headerTools">
												<span>
													...
													<a href = "administrator.php?ctg=module_hcd&op=skills&add_skill=1" title = "{$smarty.const._NEWSKILL}" >{$smarty.const._NEWSKILL}</a>
												</span>
											</div>
											{***Lesson skill table***}
<!--ajax:skillsTable-->
...
<!--/ajax:skillsTable-->
									...
									<div class="tabbertab {if (isset($smarty.post.lesson_skills) || ($smarty.get.tab == "skills"))} tabbertabdefault {/if}">
										...
										{eF_template_printBlock title = $smarty.const._LESSONSKILLSSELECTION data = $smarty.capture.t_lesson_skills image = '32x32/tools.png'}
									</div>
...
<!--ajax:branchesTable-->
										<table style = "width:100%" class = "sortedTable" size = "{$T_BRANCHES_SIZE}"  sortBy = "0" id = "branchesTable" useAjax = "1" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" url = "{$smarty.session.s_type}.php?ctg=lessons&edit_lesson={$smarty.get.edit_lesson}&">
											<tr class = "topTitle">
