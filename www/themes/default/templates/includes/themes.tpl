		            ...
		            <table class = "formElements" style = "width:100%">
						...
						<tr><td></td><td id = "toggleeditor_cell1">
							<div class = "headerTools">
								...
								<span>
									<img onclick = "javascript:toggleEditor('editor_cms_data', 'mceEditor');" class = "ajaxHandle" src = "images/16x16/order.png" title = "{$smarty.const._TOGGLEHTMLEDITORMODE}" alt = "{$smarty.const._TOGGLEHTMLEDITORMODE}" />&nbsp;
			    ...
			    {capture name = 't_cms_code'}
			        ...
			        {foreach name = 'pages_list' key = 'key' item = 'page' from = $T_CMS_PAGES}
			            ...
			            {/if}
			                ...
			                <td class = "centerAlign">
			                    <a href = "{$smarty.const.G_SERVERNAME}{$smarty.const.G_CURRENTTHEMEURL}external/{$page}.php" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._PREVIEW}', 2)"><img src = "images/16x16/search.png" title = "{$smarty.const._PREVIEW}" alt = "{$smarty.const._PREVIEW}" /></a>
			                {if $_change_}
	...
	{capture name = "t_change_theme_tab"}
		...
		<div class = "headerTools">
			<span>
				...
				<a href = "{$smarty.server.PHP_SELF}?ctg=themes&add=1&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._INSTALLTHEME}', 2)" title = "{$smarty.const._INSTALLTHEME}" target = "POPUP_FRAME">{$smarty.const._INSTALLTHEME}</a>
			</span>
		</div>
		{/if}
