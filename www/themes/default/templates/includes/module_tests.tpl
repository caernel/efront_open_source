...
*}
    ...
    {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
		{capture name = 't_create_quick_test_code_skills'}
   {*<div class = "tabbertab" id="test_options" title = "{$smarty.const._ASSOCIATEDSKILLS}">*}
   			<table>
				<tr>
					<td><a href="javascript:void(0);" onclick="add_new_criterium_row(0, 'skills')"><img src="images/16x16/add.png" title="{$smarty.const._NEWSELECTION}" alt="{$smarty.const._NEWSELECTION}"/ border="0"></a></td><td><a href="javascript:void(0);" onclick="add_new_criterium_row(0, 'skills')">{$smarty.const._NEWSELECTION}</a></td>
				</tr>
			</table>
		   	<table id = "skillsTable" class="sortedTable" width="100%" noFooter="true">
			   	<tr class = "topTitle">
			   		<td class = "topTitle noSort" width= "40%">{$smarty.const._SKILL}&nbsp;{$smarty.const._OR}&nbsp;{$smarty.const._SKILLCATEGORY}&nbsp;({$smarty.const._QUESTIONS})</td>
					<td class = "topTitle noSort" width= "40%">{$smarty.const._QUESTIONS}</td>
					<td class = "topTitle centerAlign noSort">{$smarty.const._OPERATIONS}</td>
				</tr>
   				<tr id= 'no_skills_criteria_defined'><td class= "emptyCategory" colspan="3" >{$smarty.const._SELECTASSOCIATEDSKILLSORSKILLCATEGORIES}</td></tr>
   			</table>
   {*</div>*}
		{/capture}
	{/if}	{* #cpp#endif *}
	...
	<br />
	{if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
	{eF_template_printBlock title = $smarty.const._SELECTQUESTIONSBASEDONSKILLS data = $smarty.capture.t_create_quick_test_code_skills image = '32x32/skill_gap.png'}
	<br />
	{/if} {* #cpp#endif *}
	<table width ="100%">
...
{if ($smarty.const.G_VERSIONTYPE != 'community')} {* #cpp#ifndef COMMUNITY *}
	{if ($smarty.const.G_VERSIONTYPE != 'standard')} {* #cpp#ifndef STANDARD *}
					<tr style="display:none;" id = "test_password"><td class = "labelCell">{$T_TEST_FORM.test_password.label}:&nbsp;</td>
						<td class = "elementCell">{$T_TEST_FORM.test_password.html}</td></tr>
...
<!--ajax:questionsTable-->
        <table class = "QuestionsListTable sortedTable" id = "questionsTable" size = "{$T_QUESTIONS_SIZE}" sortBy = "7" useAjax = "1" rowsPerPage = "{$smarty.const.G_DEFAULT_TABLE_SIZE}" url = "{$smarty.server.PHP_SELF}?ctg={$T_CTG}&edit_test={$smarty.get.edit_test}&tab={$smarty.get.tab}&showall={$smarty.get.showall}&">
            ...
            {if $T_CTG == 'tests' || ($T_CTG == 'feedback' && $item.type != 'true_false')}
				...
				{/if}
					<td class = "centerAlign">
						<a href = "{$smarty.server.PHP_SELF}?ctg=tests&show_question={$item.id}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._PREVIEW}', 2)"><img src = "images/16x16/search.png" alt = "{$smarty.const._PREVIEW}" title = "{$smarty.const._PREVIEW}" /></a>
						{if $T_SKILLGAP_TEST}
            ...
            {capture name = 't_user_code'}
                <div class="tabber" >
                    ...
                    {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
                        ...
                        <div class = "headerTools">
                        	<span>
                        		<img id="addToSkillSetImg" src="images/16x16/tools.png" title="{$smarty.const._ADDSKILLSTOSKILLSET}" alt="{$smarty.const._ADDSKILLSTOSKILLSET}"/>
                        		<a href="javascript:void(0);" onclick="eF_addTestSkills()">{$smarty.const._ADDSKILLSTOSKILLSET}</a>
                        	</span>
                        </div>
                        {/if}
					{/if} {* #cpp#endif *}
                        <table id="skillScoresTable" border = "0" width = "100%"  class = "sortedTable" sortBy = "0">
                            ...
                            {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
                                {if $_change_}
                                <td align="center"><a href="javascript:void(0);" onclick="eF_addSingleTestSkill('{$skill.id}')"><img id="addToSkillSetImg{$skill.id}" src="images/16x16/tools.png" title="{$smarty.const._ADDSKILLTOSKILLSET}" alt="{$smarty.const._ADDSKILLTOSKILLSET}"/ border="0"></a></td>
                                {/if}
                            {/if} {* #cpp#endif *}
                            </tr>
                            {foreachelse}
	    ...
	    <table width = "100%" class = "sortedTable">
	        ...
	        {/if}
				...
				<td class = "noWrap centerAlign">
	                ...
	                <a href = "{$smarty.server.PHP_SELF}?ctg={$T_CTG}&show_test={$test.id}&print=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._PRINT}', 2)"><img src = "images/16x16/printer.png" alt = "{$smarty.const._PRINT}" title = "{$smarty.const._PRINT}" /></a>
	  
	  {if $smarty.const.G_VERSIONTYPE != 'community'} {* #cpp#ifndef COMMUNITY *}      
			{if !isset($T_CURRENT_USER->coreAccess.statistics) || $T_CURRENT_USER->coreAccess.statistics != 'hidden'}
	                {if $T_CTG == "feedback"} 
	       	        	<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=feedback&sel_test={$test.id}"><img src = "images/16x16/reports.png" alt = "{$smarty.const._REPORTS}" title = "{$smarty.const._REPORTS}" /></a>
	       			{else}
