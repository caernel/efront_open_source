		...
		{foreach name = "folders_loop" key = "id" item = "folder" from = $T_FOLDERS}
		    <tr id = "div_folder_id_{$id}">
		        ...
		        {if $smarty.foreach.folders_loop.iteration > 3}
					<a href = "{$smarty.server.PHP_SELF}?ctg=messages&folders=1&edit={$id}&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDIT}', 2)" target = "POPUP_FRAME"><img class = "handle" src = "images/16x16/edit.png" title = "{$smarty.const._EDIT}" alt = "{$smarty.const._EDIT}" /></a>
            ...
            <div id = "additional_recipients_categories" style="display:none;">
                <div>
                    ...
                    {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
                        ...
                        {* Only admin *}
                        <tr {if $smarty.session.s_type != "administrator"}style="display:none;"{/if} ><td>{$T_ADD_MESSAGE_FORM.recipients.active_users.html}   </td><td colspan=2 align="left">{$smarty.const._ALLACTIVESYSTEMUSERS}</td></tr>
	...
	{elseif $smarty.get.view}
		{capture name = "t_messagesbody_code"}
			<div class = "messagesTable" >
				...
				{/if}
				{if $_change_}
					{if $smarty.session.s_type != 'student' || 'messages'|eF_template_isOptionVisible}
...
<!--/ajax:messagesTable-->
         	<div class = "horizontalSeparatorAbove">
				...
				<img class = "ajaxHandle" src = "images/16x16/file_explorer.png" title = "{$smarty.const._MOVETOFOLDER}" alt = "{$smarty.const._MOVETOFOLDER}" onclick = "moveMessages(this)"/>
                <select id = "target_message_folder">{$folders_options}</select>             
             </div>
		{/capture}
