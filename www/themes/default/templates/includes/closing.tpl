...
{/if}
{if isset($div_error)}
	eF_js_showDivPopup(event, '{$div_error}');
{/if}
...
<script>
{* because of http://stackoverflow.com/questions/7693663/remove-from-the-facebook-redirect-url *}
{literal}
if (window.location.href.indexOf("#_=_") > 0) { window.location = window.location.href.replace(/#.*/, ""); }
{/literal}
</script>

{if $T_CONFIGURATION.facebook_api_key}     	      
	{literal}        	
	  <script>
	    window.fbAsyncInit = function() {
	      FB.init({
	        appId      : "{/literal}{$T_CONFIGURATION.facebook_api_key}{literal}",
	        status     : true, 
	        cookie     : true,
	        xfbml      : true,
	        oauth      : true,
	        ...
	        $$('.fb_login').each(function(element) {
	    	    element.observe('click', fcbLoginHandler);
	    	})
	
	       function fcbLoginHandler(event) {
	    ...
	    function facebook_logout() {
	    	 FB.getLoginStatus(function(response) {
	    		 if (response.status === 'connected') { 
			    		...
			    		window.location = "{/literal}{$smarty.const.G_SERVERNAME}{literal}index.php?logout=true"; 
		        	});
	    		} else {
	    			window.location = "{/literal}{$smarty.const.G_SERVERNAME}{literal}index.php?logout=true"; 
		    	}
