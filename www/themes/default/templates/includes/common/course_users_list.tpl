...
{if $smarty.const.G_VERSIONTYPE != 'community'} {* #cpp#ifndef COMMUNITY *}
		...
		{if $smarty.const.G_VERSIONTYPE != 'standard'} {* #cpp#ifndef STANDARD *}
			{if in_array('expire_certificate', $T_DATASOURCE_COLUMNS)}<td class = "topTitle expire_certificate noSort" name = "expire_certificate">{$smarty.const._CERTIFICATEEXPIRESON}</td>{/if}
		{/if} {* #cpp#endif *}
{/if} {* #cpp#endif *}
...
{if in_array('operations', $T_DATASOURCE_COLUMNS)}
	...
	{if !isset($T_DATASOURCE_OPERATIONS) || in_array('certificate', $T_DATASOURCE_OPERATIONS)}
		...
		{if (!isset($T_CURRENT_USER->coreAccess.course_settings) || $T_CURRENT_USER->coreAccess.course_settings == 'change')}
			{if $user.issued_certificate}
				<a href = "{$smarty.server.PHP_SELF}?{$T_BASE_URL}&op=course_certificates&export={$certificate_export_method}&user={$user.login}&course={$smarty.get.course}" target="_blank" title = "{$smarty.const._VIEWCERTIFICATE}">
					<img src = "images/16x16/certificate.png" title = "{$smarty.const._VIEWCERTIFICATE}" alt = "{$smarty.const._VIEWCERTIFICATE}" border = "0"/>
				</a>&nbsp;
				<a href = "{$smarty.server.PHP_SELF}?{$T_BASE_URL}&op=course_certificates&revoke_certificate={$user.login}" title = "{$smarty.const._REVOKECERTIFICATE}">
					<img src = "images/16x16/error_delete.png" title = "{$smarty.const._REVOKECERTIFICATE}" alt = "{$smarty.const._REVOKECERTIFICATE}" border = "0"/>
				</a>&nbsp;
				{if $user.completed}	
					<a href = "{$smarty.server.PHP_SELF}?{$T_BASE_URL}&op=course_certificates&reset_keep={$user.login}" title = "{$smarty.const._RESETPROGRESSDATAKEEPCERTIFICATE}">
						<img src = "images/16x16/refresh.png" title = "{$smarty.const._RESETPROGRESSDATAKEEPCERTIFICATE}" alt = "{$smarty.const._RESETPROGRESSDATAKEEPCERTIFICATE}" border = "0"/>
					</a>&nbsp;
				{/if}	
			{elseif $user.completed}
				<a href = "{$smarty.server.PHP_SELF}?{$T_BASE_URL}&op=course_certificates&issue_certificate={$user.login}&popup=1" title = "{$smarty.const._ISSUECERTIFICATE}">
			...
			{else}
				<a href = "javascript:void(0)">
					<img class = "inactiveImage" src = "images/16x16/certificate.png" title = "{$smarty.const._THEUSERHASNOTCOMPLETEDTHELESSON}" alt = "{$smarty.const._THEUSERHASNOTCOMPLETEDTHELESSON}" />&nbsp;
				</a>
			{/if}
		{else}
			{if $user.issued_certificate}
