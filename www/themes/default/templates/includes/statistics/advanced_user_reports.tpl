{capture name = "t_users_table_code"}
	...
	{else}
	<div>{$smarty.const._NOREPORTSFOUNDINTHESYSTEM}. {if $smarty.session.s_type=='administrator'}<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&tab=builder&add=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDREPORT}', 0)">{$smarty.const._YOUMAYCREATEONE}</a>{/if}</div>
	{/if}
	{if $T_REPORT}
			...
			{foreach name = 't_columns_list' item = "item" key = "foo" from = $T_REPORT.rules.columns}
				...
				<td style = "{if $item.width}width:{$item.width}%;{/if}{if $item.align}text-align:{$item.align};{/if}">
					...
					{if $item.column == $T_EDIT_LINK}
						<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=profile" class = "editLink {if 'tooltip'|eF_template_isOptionVisible}info{/if}" url = "ask_information.php?users_LOGIN={$user.login}&type=user">
					{/if}
					{if $item.column == 'branch'}
						{if !$T_EDIT_LINK}
							<a href = "{$smarty.server.PHP_SELF}?ctg=module_hcd&op=branches&edit_branch={$user.branch_ID}" class = "editLink {if 'tooltip'|eF_template_isOptionVisible}info{/if}" url = "ask_information.php?users_LOGIN={$user.login}&type=user">{$entry} {if $user.sum_branch > 1}({$user.sum_branch-1} {$smarty.const._MORE}){/if}
						{else}
					...
					{elseif $item.column == 'course_status'}
						{if $user.count_courses}
							<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=user_courses" class = "editLink {if 'tooltip'|eF_template_isOptionVisible}info{/if}" url = "ask_information.php?users_LOGIN={$user.login}&type=course_status">
								{$user.course_status}
								{if 'tooltip'|eF_template_isOptionVisible}
					...
					{elseif $item.column == 'lesson_status'}
						{if $user.count_lessons}
							<a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$user.login}&op=user_lessons" class = "editLink {if 'tooltip'|eF_template_isOptionVisible}info{/if}" url = "ask_information.php?users_LOGIN={$user.login}&type=course_status">
								{$user.lesson_status}
								{if 'tooltip'|eF_template_isOptionVisible}
									<span class = "tooltipSpan"></span>
								{/if}
...
{capture name = "t_report_builder_code"}
	<div class = "headerTools">
		...
		<span>
			...
			<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&add=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._REPORT}', 0)">{$smarty.const._ADDREPORT}</a>
		</span>
		{if $T_REPORT_NAMES}
		<span>
	...
	{if $T_REPORT}
			<div class = "mediumHeader">
				...
				<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&edit={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._REPORT}', 0)">
			...
			<div class = "headerTools">
				<span>
					...
					<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&add_condition=1&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDNEWCONDITION}', 3)">{$smarty.const._ADDNEWCONDITION}</a>
				...
				<span>
					...
					<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&order_condition=1&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._CHANGECONDITIONORDER}', 2)">{$smarty.const._CHANGECONDITIONORDER}</a>
				</span>
			</div>
...
<!--ajax:conditionsTable-->
	<table id = "conditionsTable" style = "width:100%" size = "{$T_TABLE_SIZE}" class = "sortedTable" useAjax = "1" url = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&report={$smarty.get.report}&">
				...
				<tr class = "defaultRowHeight {cycle values = "oddRowColor, evenRowColor"}">
					...
					<td class = "centerAlign">
						<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&edit_condition={$key}&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._REPORTSCONDITION}', 3)">
...
{*
				<span>
					...
					<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&add_column=1&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ADDCOLUMN}', 2)">{$smarty.const._ADDCOLUMN}</a>
				</span>
*}
				<span>
					<img src = "images/16x16/order.png" alt = "{$smarty.const._CHANGECOLUMNORDER}" title = "{$smarty.const._CHANGECOLUMNORDER}" />
					<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&order_column=1&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._CHANGECOLUMNORDER}', 2)">{$smarty.const._CHANGECOLUMNORDER}</a>
				</span>
			</div>
			...
			<table class = "sortedTable" style = "width:100%" id = "columns_table">
				...
				<tr class = "defaultRowHeight {cycle values = "oddRowColor, evenRowColor"}">
					...
					<td class = "centerAlign">
						<a href = "{$smarty.server.PHP_SELF}?ctg=statistics&option=advanced_user_reports&edit_column={$key}&report={$smarty.get.report}&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._EDITCOLUMN}', 3)">
							<img class = "ajaxHandle" src = "images/16x16/edit.png" alt = "{$smarty.const._EDIT}" title = "{$smarty.const._EDIT}" onclick = "eF_js_showDivPopup();"/></a>
					</td>
				</tr>
				{foreachelse}
