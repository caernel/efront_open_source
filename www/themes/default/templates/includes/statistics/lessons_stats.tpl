        ...
        {else}
				...
				<tr>
                    ...
                    <td id = "right">
                        ...
                        <a href = "{$T_BASIC_TYPE}.php?ctg=statistics&option=lesson&sel_lesson={$smarty.get.sel_lesson}&group_filter={$smarty.get.group_filter}&excel=lesson&branch_filter={$smarty.get.branch_filter}&job_filter={$smarty.get.job_filter}&subbranches={$smarty.get.subbranches}&user_filter={$smarty.get.user_filter}">
                            <img src = "images/file_types/xls.png" title = "{$smarty.const._XLSFORMAT}" alt = "{$smarty.const._XLSFORMAT}"/>
                        </a>
                        <a href = "{$T_BASIC_TYPE}.php?ctg=statistics&option=lesson&sel_lesson={$smarty.get.sel_lesson}&group_filter={$smarty.get.group_filter}&pdf=lesson&branch_filter={$smarty.get.branch_filter}&job_filter={$smarty.get.job_filter}&subbranches={$smarty.get.subbranches}&user_filter={$smarty.get.user_filter}">
...
<!--/ajax:lessonUsersTable-->
	{/if}
                ...
                {if !empty($T_TESTS_INFO) && 'tests'|eF_template_isOptionVisible}
                ...
                {foreach key = "test_id" item = "test_info" from = $T_TESTS_INFO}
					...
					<table class = "statisticsTools">
                        <tr><td>
                            ...
                            <td id = "right">
                                <a href = "javascript:void(0)" onclick = "eF_js_showDivPopup(event, '{$smarty.const._QUESTIONTYPES}', 2, 'graph_table');showGraph($('proto_chart'), 'graph_test_questions', '{$test_info.general.id}');">{$smarty.const._QUESTIONSKIND}: </a>
                                <img class = "ajaxHandle" src = "images/16x16/reports.png" alt = "{$smarty.const._QUESTIONSKIND}" title = "{$smarty.const._QUESTIONSKIND}" onclick = "eF_js_showDivPopup(event, '{$smarty.const._QUESTIONTYPES}', 2, 'graph_table');showGraph($('proto_chart'), 'graph_test_questions', '{$test_info.general.id}');"/>
                            </td>
                    </table>
                    ...
                    <table class = "sortedTable" sortBy = "0">
                        ...
                        <tr class = "{cycle name = $test_id values = "oddRowColor, evenRowColor"}">
                        	...
                        	<td class = "centerAlign">
                        		{if !$test_info.general.scorm}
                                <a href = "view_test.php?done_test_id={$done_test.active_test_id}&popup=1" onclick = "eF_js_showDivPopup(event, '{$smarty.const._VIEWTEST}', 3)" target = "POPUP_FRAME">
                ...
                {if !empty($T_QUESTIONS_INFORMATION) && 'tests'|eF_template_isOptionVisible}
                ...
                {if !empty($T_PROJECTS_INFORMATION) && 'projects'|eF_template_isOptionVisible}
                ...
                <div class = "statisticsDiv tabbertab {if (isset($smarty.get.tab) &&  $smarty.get.tab == 'overall')} tabbertabdefault{/if}" title = "{$smarty.const._MOREINFO}">
                    <table class = "statisticsGeneralInfo">
						...
						{if 'comments'|eF_template_isOptionVisible}
						...
						{/if}
						{if 'forum'|eF_template_isOptionVisible}
							<tr class = "defaultRowHeight {cycle name = 'participation_lesson_info' values = 'oddRowColor, evenRowColor'}">
...
{*
                    <table class = "statisticsTools">
                        <tr><td id = "right">
								{$smarty.const._ACCESSSTATISTICS}: <img class = "handle" src = "images/16x16/reports.png" alt = "{$smarty.const._ACCESSSTATISTICS}" title = "{$smarty.const._ACCESSSTATISTICS}" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ACCESSSTATISTICS}', 2, 'graph_table');showGraph($('proto_chart'), 'graph_access');"/>
							</td>
                        </tr>
                    </table>
*}
...
{*                                
                                <td class = "centerAlign">
                                    <img class = "handle" src = "images/16x16/reports.png" alt = "{$smarty.const._ACCESSSTATISTICS}" title = "{$smarty.const._ACCESSSTATISTICS}" onclick = "eF_js_showDivPopup(event, '{$smarty.const._ACCESSSTATISTICS}', 2, 'graph_table');showGraph($('proto_chart'), 'graph_user_access', '{$login}');"/>
                                </td>
*}                                
                            </tr>
                        {foreachelse}
