...
{if $smarty.session.s_lessons_ID}
    ...
    {if $T_NO_HORIZONTAL_MENU}{assign var = "title_onclick" value = "top.sideframe.hideAllLessonSpecific();"}{/if}
		...
		{*We use &nbsp;&raquo;&nbsp; for text in title path but &nbsp;&rarr;&nbsp; for onmouseover info in order to preserve eF_formatTitlePath*}		
			...
			{assign var = "title" value = "`$title`<span>&nbsp;&raquo;&nbsp;</span><a class = 'titleLink' title = '`$titleCourse`' href ='`$smarty.server.PHP_SELF`?ctg=control_panel'>`$T_CURRENT_COURSE_NAME`</a>"}
		{/if}
		{if $T_CURRENT_LESSON}
...
{capture name = "t_path_additional_code"}
	...
	{if $smarty.session.s_lessons_ID != '' && 'bookmarks'|eF_template_isOptionVisible && $T_CURRENT_LESSON->options.bookmarking}
...
{if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
	{if ($smarty.get.print == 1)}
	    {if $T_CTG == 'users'}
	        ...
	        {/literal}
	    {else if $T_CTG == 'module_hcd' && $T_OP == 'chart'}
	        {literal}
	        <script>
	        printPartOfPage('singleColumn');
	        </script>
