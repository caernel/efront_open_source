...
{*moduleStatistics: The administrator statistics page*}
	...
	{elseif $smarty.get.option == 'branches'}
		...
		{/if}
	{elseif $smarty.get.option == 'skill'}
        {assign var = "title" value = $title|cat:'&nbsp;&raquo;&nbsp;<a class = "titleLink" href = "'|cat:$smarty.server.PHP_SELF|cat:'?ctg=statistics&option=skill">'|cat:$smarty.const._SKILLSSTATISTICS|cat:'</a>'}
		{if isset($smarty.get.sel_skill)}
			{assign var = "title" value = $title|cat:'&nbsp;&raquo;&nbsp;<a class = "titleLink" href = "'|cat:$smarty.server.PHP_SELF|cat:'?ctg=statistics&option=skill&sel_skill='|cat:$smarty.get.sel_skill|cat:'">'|cat:$T_CURRENT_SKILL_INFO->skill.description|cat:'</a>'}
		{/if}
    ...
    {capture name = "moduleSearchCoursesPage"}
                            <tr><td class = "moduleCell">
                                ...
                                {if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
                                    ...
                                    <br />
                                    {eF_template_printBlock title=$smarty.const._EMPLOYEESFULFILLINGCRITERIA data=$smarty.capture.t_found_employees_code image='32x32/user.png' options = $T_SENDALLMAIL_LINK}
                                {else} {* #cpp#else *}
                                    {eF_template_printBlock title=$smarty.const._FINDEMPLOYEES data=$smarty.capture.t_search_course_code image='32x32/scorm.png' main_options=$T_TABLE_OPTIONS}
                                    <br />
...
{if $smarty.const.G_VERSIONTYPE == 'enterprise'} {* #cpp#ifdef ENTERPRISE *}
    {if ($smarty.get.print == 1)}
        {if $T_CTG == 'users'}
            <script>printPartOfPage('singleColumn')</script>
