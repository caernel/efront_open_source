...
{/if}
{if (isset($T_CTG) && $T_CTG == 'tincan')}
	{assign var = "title" value = "`$title`&nbsp;&raquo;&nbsp;<a class = 'titleLink'  href = '`$smarty.server.PHP_SELF`?ctg=tincan'>`$smarty.const._TINCAN`</a>"}
	{include file = "includes/tincan.tpl"}
{/if}
...
{if ($T_CTG == 'statistics')}
	...
	{* #cpp#ifdef ENTERPRISE *}
		{assign var = "title" value = $title|cat:'&nbsp;&raquo;&nbsp;<a class = "titleLink" href = "'|cat:$smarty.server.PHP_SELF|cat:'?ctg=statistics&option=branches">'|cat:$smarty.const._BRANCHSTATISTICS|cat:'</a>'}
		{if isset($smarty.get.sel_branch)}
			{assign var = "title" value = $title|cat:'&nbsp;&raquo;&nbsp;<a class = "titleLink" href = "'|cat:$smarty.server.PHP_SELF|cat:'?ctg=statistics&option=branches&sel_branch='|cat:$smarty.get.sel_branch|cat:'">'|cat:$T_BRANCH_INFO.name|cat:'</a>'}
		{/if}
    {elseif $smarty.get.option == 'tincan'}
        {assign var = "title" value = $title|cat:'&nbsp;&raquo;&nbsp;<a class = "titleLink" href = "'|cat:$smarty.server.PHP_SELF|cat:'?ctg=statistics&option=tincan">'|cat:$smarty.const._TINCANREPORTS|cat:'</a>'}
	{* #cpp#endif *}
...
{*moduleHCD: Show evaluations for this employee *}
		...
		{assign var = 'title' value = "`$title`&nbsp;&raquo;&nbsp;<a class = 'titleLink' href ='professor.php?ctg=settings'>`$smarty.const._OPTIONSFORLESSON` &quot;`$T_CURRENT_LESSON->lesson.name`&quot;</a>"}
		{assign var = 'title' value = "`$title`&nbsp;&raquo;&nbsp;<a class = 'titleLink' href ='`$smarty.server.PHP_SELF`?ctg=settings&op=lesson_users'>`$smarty.const._LESSONUSERS`</a>"}
		{assign var = 'title' value = "`$title`&nbsp;&raquo;&nbsp;<a class='titleLink' href ='`$smarty.server.PHP_SELF`?ctg=evaluations&user=`$smarty.get.user`'>`$smarty.const._EVALUATIONS`</a>"}

		{capture name = 't_employee_evaluations_code'}

		   <table>
			   <tr>
					<td><a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$smarty.get.user}&op=evaluations&add_evaluation=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWEVALUATION}', 1)"><img src="images/16x16/add.png" title="{$smarty.const._NEWEVALUATION}" alt="{$smarty.const._NEWEVALUATION}" border="0" /></a></td>
					<td><a href = "{$smarty.server.PHP_SELF}?ctg=personal&user={$smarty.get.user}&op=evaluations&add_evaluation=1&popup=1" target = "POPUP_FRAME" onclick = "eF_js_showDivPopup(event, '{$smarty.const._NEWEVALUATION}', 1)">{$smarty.const._NEWEVALUATION}</a></td>
			   </tr>
		   </table>
				...
				<tr class = "{cycle values = "oddRowColor, evenRowColor"}">
					...
					<td align = "center">
						<table>
							<tr>
							<td width="45%">
								<a href = "{$smarty.session.s_type}.php?ctg=personal&user={$smarty.get.user}&op=evaluations&edit_evaluation={$evaluation.event_ID}" class = "editLink"><img border = "0" src = "images/16x16/edit.png" title = "{$smarty.const._EDIT}" alt = "{$smarty.const._EDIT}" /></a></td><td>
							</td>
							<td  width="45%">
								<a href = "{$smarty.session.s_type}.php?ctg=personal&user={$smarty.get.user}&op=evaluations&delete_evaluation={$evaluation.event_ID}" onclick = "return confirm('{$smarty.const._AREYOUSUREYOUWANTTODELETEEVALUATION}')" class = "deleteLink"><img border = "0" src = "images/16x16/error_delete.png" title = "{$smarty.const._DELETE}" alt = "{$smarty.const._DELETE}" /></a>
							</td>
							</tr>
						</table>
					</td>
		...
		{/capture}
		{eF_template_printBlock title = $smarty.const._EVALUATIONOFEMPLOYEE|cat:'&nbsp;'|cat:$smarty.get.user data = $smarty.capture.t_employee_evaluations_code image = '32x32/catalog.png'}
		{/capture}
{/if}
...
{capture name = "center_code"}
	...
	<table class = "centerTable">
		...
		{$smarty.capture.moduleTincanOptions}
...
{capture name = "t_path_additional_code"}
	...
	{if $smarty.session.s_lessons_ID != '' && 'bookmarks'|eF_template_isOptionVisible}
...
{if ($smarty.get.print == 1)}
	{if $T_CTG == 'users'}
		...
		{/literal}
	{elseif $T_CTG == 'module_hcd' && $T_OP == 'chart'}
		{literal}
		<script>
		printPartOfPage('singleColumn');
		</script>
