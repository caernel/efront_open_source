...
class EfrontNotification
{
	...
	const USERSUPERVISORS = 9;		// all users that supervise the branches of the user (also the father branches recursively)
	const ALLCOURSEUSERS = 10;		// all course professors 
	const ALLSUPERVISORS = 11;		// all users that supervise branches 
	const USERIMMEDIATESUPERVISORS = 12;	// all users that supervise the branches of the user
    /**
    ...
    public static function getNotificationTypes($get_module_notifications = false) {
   		$system_notifications = array(EfrontNotification::SYSTEM_JOIN => array("text" => _SYSTEMJOIN, "category" => "system"),
   			...
   			if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
       			$system_notifications[EfrontNotification::HCD_NEW_BRANCH] = array("text"=>_ONNEWBRANCH, "category" => "branch");
       			$system_notifications[EfrontNotification::HCD_REMOVE_BRANCH] = array("text"=>_ONBRANCHREMOVAL, "category" => "branch");
       			$system_notifications[EfrontNotification::HCD_NEW_JOB_ASSIGNMENT] = array("text"=>_ONJOBASSIGNMENT, "category" => "job");
       			$system_notifications[EfrontNotification::HCD_REMOVE_JOB_ASSIGNMENT] = array("text"=>_ONJOBREMOVAL, "category" => "job");
   			}  #cpp#endif
	...
	public static function addDefaultNotifications() {
		...
		if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
			if (!in_array(EfrontEvent::NEW_SURVEY, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::NEW_SURVEY,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::EXPLICITLYSEL,
											  "subject"    		=> _NEWSURVEYPUBLISHED . " ###survey_name###",
											  "html_message"	=> 1,
											  "message"			=> _DEARUSER." ###users_name###,<br><br>"._THISISANAUTOMATEDEMAILSENTFROM." ###host_name###<br><br>".
	                                							   "###survey_message###<br><br><a href='###host_name###/external_survey.php?username=###users_login###&coupon=###md5(###users_login######survey_id###)###&surveys_ID=###survey_id###'>"._DOSURVEY."<br/>###host_name###/external_survey.php?username=###users_login###&coupon=###md5(###users_login######survey_id###)###&surveys_ID=###survey_id###</a><br>"._THANKYOU);

				eF_insertTableData("event_notifications", $default_notification);
			}


			if (!in_array(EfrontEvent::NEW_SYSTEM_ANNOUNCEMENT, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::NEW_SYSTEM_ANNOUNCEMENT,
										      "send_conditions" => serialize(array()),
				 							  "send_recipients" => EfrontNotification::ALLSYSTEMUSERS,
											  "subject"    		=> _NEWSYSTEMANNOUNCEMENT,
											  "html_message"	=> 1,
											  "message"			=> _EMAILSENTFROM.' ###host_name### '._TOALLSYSTEMUSERS.
	                   											   ' '._ON.' ###date###<br><br>###announcement_title###<br>---------------------------------------<br>###announcement_body###<br>');

				eF_insertTableData("event_notifications", $default_notification);
			}

			if (!in_array(EfrontEvent::NEW_LESSON_ANNOUNCEMENT, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::NEW_LESSON_ANNOUNCEMENT,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::ALLLESSONUSERS,
											  "subject"    		=> _LESSONANNOUNCEMENT . " ###lessons_name###",
											  "html_message"	=> 1,
											  "message"			=> _EMAILSENTFROM.' ###host_name### '._TO.' '._STUDENTSOFLESSONSMALL.' ###lessons_name###'.
	                   												' '._ON.' ###date###<br><br>###announcement_title###<br>---------------------------------------<br>###announcement_body###<br>');
			...
			// Inactive events
			$system_not_visited_event_type = (-1) * EfrontEvent::SYSTEM_VISITED;
			if (!in_array($system_not_visited_event_type, $registered_events)) {
				$default_notification = array("event_type" 		=> $system_not_visited_event_type,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::TRIGGERINGUSER,
											  "after_time"		=> 604800,
											  "active"			=> 0,
											  "subject"    		=> _WEEKLYREMINDERFROM . " ###site_name###",
											  "message"			=> _DEARUSER." ###users_name###,<br><br>"._THISISANAUTOMATEDEMAILSENTFROM." ###host_name###<br><br>".
																   _YOUHAVENOTLOGGEDINFORAWEEK . ". " . _USETHEFOLLOWINGLINKTOLOGINTONOW . " ###host_name###/index.php" . ". " .
																   _FORFURTHERCONTACTADMINAT." ###host_name###/index.php?ctg=contact <br><br>"._KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			if (!in_array(EfrontEvent::SYSTEM_JOIN, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::SYSTEM_JOIN,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::SYSTEMADMINISTRATOR,
											  "active"			=> 0,
											  "subject"    		=> _NEWSYSTEMJOININ . " ###site_name###",
											  "message"			=> _ANEWUSERHASBEENINSERTEDINTOTHESYSTEM . "<br>Login: ###triggering_users_login###<br>Name: ###triggering_users_name### ###triggering_users_surname###<br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			if (!in_array(EfrontEvent::TEST_COMPLETION, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::TEST_COMPLETION,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::LESSONPROFESSORS,
											  "active"			=> 0,
											  "subject"    		=> _TESTCOMPLETIONFORLESSON . " ###lessons_name###",
											  "message"			=> _STUDENTOFLESSON . " ###lessons_name### " . _HASSUBMITTEDATEST . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}
			if (!in_array(EfrontEvent::TEST_FAILURE, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::TEST_FAILURE,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::LESSONPROFESSORS,
											  "active"			=> 0,
											  "subject"    		=> _TESTFAILUREFORLESSON . " ###lessons_name###",
											  "message"			=> _STUDENTOFLESSON . " ###lessons_name### " . _HASFAILEDATEST . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}
			if (!in_array(EfrontEvent::PROJECT_SUBMISSION, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::PROJECT_SUBMISSION,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::LESSONPROFESSORS,
											  "active"			=> 0,
											  "subject"    		=> _PROJECTCOMPLETIONFORLESSON . " ###lessons_name###",
											  "message"			=> _STUDENTOFLESSON . " ###lessons_name### " . _HASSUBMITTEDAPROJECT . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			// Send 3 days before project expiration
			$project_expiry_approaching_type = (-1) * EfrontEvent::PROJECT_EXPIRY;
			if (!in_array($project_expiry_approaching_type, $registered_events)) {
				$default_notification = array("event_type" 		=> $project_expiry_approaching_type,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::EXPLICITLYSEL,
											  "after_time"		=> -259200,
											  "active"			=> 0,
											  "subject"    		=> _PROJECTEXPIRATIONFORLESSON . " ###lessons_name###",
											  "message"			=> _DEARUSER . " ###users_name###,<br><br>" . _THEPROJECTFORLESSON . " ###lessons_name### " . _ISABOUTTOEXPIREINTHREEDAYS . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			$lesson_expiry_approaching_type = (-1) * EfrontEvent::LESSON_PROGRAMMED_EXPIRY;
			if (!in_array($lesson_expiry_approaching_type, $registered_events)) {
				$default_notification = array("event_type" 		=> $lesson_expiry_approaching_type,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::ALLLESSONUSERS,
											  "after_time"		=> -259200,
											  "active"			=> 0,
											  "subject"    		=> _LESSONEXPIRATIONFORLESSON . " ###lessons_name###",
											  "message"			=> _DEARUSER . " ###users_name###,<br><br>" . _LESSON . " ###lessons_name### " . _ISABOUTTOEXPIREINTHREEDAYS . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}


			if (!in_array(EfrontEvent::NEW_TOPIC, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::NEW_TOPIC,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::ALLLESSONUSERS,
											  "active"			=> 0,
											  "subject"    		=> _NEWTOPICFORFORUM . " ###lessons_name###",
											  "message"			=> _NEWTOPICPUBLISHEDINTHEFORUMOF . " ###lessons_name### " . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			if (!in_array(EfrontEvent::NEW_FORUM_MESSAGE_POST, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::NEW_FORUM_MESSAGE_POST,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::ALLLESSONUSERS,
											  "active"			=> 0,
											  "subject"    		=> _NEWMESSAGEPOSTFORFORUM . " ###lessons_name###",
											  "message"			=> _NEWMESSAGEPOSTPUBLISHEDINTHEFORUMOF . " ###lessons_name### " . "<br><br>". _KINDREGARDSEFRONT."<br>---<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");

				eF_insertTableData("event_notifications", $default_notification);
			}

			if (!in_array(EfrontEvent::CONTENT_MODIFICATION, $registered_events)) {
				$default_notification = array("event_type" 		=> EfrontEvent::CONTENT_MODIFICATION,
										      "send_conditions" => serialize(array()),
											  "send_recipients" => EfrontNotification::ALLLESSONUSERS,
											  "active"			=> 0,
											  "subject"    		=> "###site_name###: " . _CHANGEINUNIT . " \"###unit_title###\" " . _OFLESSON . "\"###lessons_name###\"",
											  "html_message"	=> 1,
											  "message"			=> _DEARUSER . " ###users_name###,<br><br>" . _UNITTITLED . " \"###unit_title###\" ". _HASBEENMODIFIEDAS. ":<br><br><table border=1><tr><td>###unit_content###</td></tr></table><br><br>"._KINDREGARDSEFRONT."<br>--<br>"._ADMINISTRATIONGROUP."<br>###site_name###<br>###site_motto###<br>");
    	...
    	if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
    		if (G_VERSIONTYPE != 'standard') { #cpp#ifndef STANDARD
		    	...
		    	$notification = array ("event_type"		 => $events_type,
		    						   "send_conditions" => serialize($condition),
		    						   "send_recipients" => $recipients_category ? $recipients_category : 0,
		    						   "message" 		 => $message,
		    						   "subject" 		 => $subject,
		    						   "html_message"	 => $html_message ? $html_message : 0,
		    	                       "send_immediately"=> $send_immediately? $send_immediately : 0);
			    	...
			    	if (isset($this -> recipients["lessons_ID"]) && $this -> recipients["lessons_ID"]) {
			            ...
			            if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
			            	if (isset($this -> recipients['supervisor'])) {
								...
								$result = eF_getTableDataFlat("module_hcd_employee_works_at_branch", "users_LOGIN", "supervisor=1 and branch_ID IN (select branch_ID from module_hcd_employee_works_at_branch where assigned=1 and supervisor=0 and users_login='".$this -> recipients['users_login']."')");
...
//		EfrontNotification::initializeEventNotification($event_notification[0]);
    ...
    public static function getAllNotifications() {
    	...
    	foreach ($event_notifications as $notification) {
    		...
    		} else if ($notification['send_recipients'] == EfrontNotification::USERIMMEDIATESUPERVISORS) {
   ...
   /**
    ...
    public function createMessage($modulesArray = false) {
	    ...
	    if ($this -> notification['type'] >= EfrontNotification::MODULE_BASE_TYPE_CODE) {
	        ...
	        $this -> notification['message'] = _NAMEARTICLE . " <b><a  href = \"".$currentUser -> getType().".php?ctg=social&op=show_profile&user=".$this->notification['users_LOGIN']. "&popup=1\" onclick = \"eF_js_showDivPopup(event, '" . _USERPROFILE . "', 1)\"  target = \"POPUP_FRAME\"> ". $this -> notification['users_name']. " " . $this -> notification['users_surname']. "</a></b> ";
