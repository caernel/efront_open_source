...
*/
	...
	public static function getQuestionsStatistics($test = false, $users = false) {
	    ...
	    if (!$test) {
	        $result = EfrontCompletedTest::retrieveCompletedTest("completed_tests ct, completed_tests_blob ctb, users u", "ct.*,ctb.test", "ct.users_LOGIN=u.login and ct.id=ctb.completed_tests_ID and u.archive=0 and ct.status != 'deleted' and ct.status != 'incomplete' $sql");
	    } else {
	        ...
	        $result = EfrontCompletedTest::retrieveCompletedTest("completed_tests ct, completed_tests_blob ctb, users u", "ct.*,ctb.test", "ct.users_LOGIN=u.login and ct.id=ctb.completed_tests_ID and u.archive=0 and ct.status != 'deleted'  and ct.status != 'incomplete' and tests_ID = $testId $sql");
	    }
