...
//2222222222222222222222222
	   			...
	   			if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
	   				$system_events[EfrontEvent::HCD_NEW_BRANCH] = array("text"=>_ONNEWBRANCH, "category" => "branch");
	   				$system_events[EfrontEvent::HCD_REMOVE_BRANCH] = array("text"=>_ONBRANCHREMOVAL, "category" => "branch");
	   				$system_events[EfrontEvent::HCD_NEW_JOB_ASSIGNMENT] = array("text"=>_ONJOBASSIGNMENT, "category" => "job");
	   				$system_events[EfrontEvent::HCD_REMOVE_JOB_ASSIGNMENT] = array("text"=>_ONJOBREMOVAL, "category" => "job");
	   				$system_events[EfrontEvent::HCD_NEW_SKILL] = array("text"=>_ONNEWSKILL, "category" => "skill");
	   				$system_events[EfrontEvent::HCD_REMOVE_SKILL] = array("text"=>_ONSKILLREMOVAL, "category" => "skill");
	   				$system_events[EfrontEvent::HCD_SKILL_EDIT] = array("text"=>_ONSKILLEDIT, "category" => "skill");
	   				$system_events[EfrontEvent::HCD_HIRED] = array("text"=> _ONHCDHIRED, "category" => "hcd");
	   				$system_events[EfrontEvent::HCD_LEFT] = array("text"=> _ONHCDLEFT, "category" => "hcd");
	   				$system_events[EfrontEvent::HCD_WAGE_CHANGE] = array("text"=> _ONWAGECHANGE, "category" => "hcd");
    			...
    			if ($conditions_passed) {
		            ...
		            if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
		            	if ($event_notification['send_recipients'] == EfrontNotification::USERSUPERVISORS) {
							...
							$event_notification['send_conditions'] = serialize($cond);
							$event_notification['recipient'] = "";
		            	} else if ($event_notification['send_recipients'] == EfrontNotification::ALLSUPERVISORS) {
	    ...
	    if ($this -> event['type'] >= EfrontEvent::MODULE_BASE_TYPE_CODE) {
	        ...
	        // For courses we have lessons_name -> courses_name
	            ...
	            if ($this -> event['entity_ID'] != $currentUser -> user['login']) {
	            	$this -> event['message'] .=  " <b><a  href = \"".$currentUser -> getType().".php?ctg=social&op=show_profile&user=".$this->event['entity_ID']. "&popup=1\" onclick = \"eF_js_showDivPopup(event, '" . _USERPROFILE . "', 1)\"  target = \"POPUP_FRAME\"> ". $this -> event['entity_name']. "</a></b> ";
	            } else {
			...
			} else if ($this -> event['type'] == EfrontEvent::NEW_MANUAL_PAYMENT) {
