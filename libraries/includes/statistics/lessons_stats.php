...
if (isset($_GET['excel']) && $_GET['excel'] == 'lesson') {
    ...
    $constraints   = array('return_objects' => false, 'table_filters' => $stats_filters);
    ...
    if ($groupname || $branchName || $_GET['user_filter']) {   	
    ...
    if (sizeof($lessonProjects) > 0 && EfrontUser::isOptionVisible('projects')) {
...
/*
        $row++;
        if (sizeof($doneTests[$user['login']]) > 0 && EfrontUser::isOptionVisible('tests')) {
            $avgScore = array();
        ...
        if (sizeof($assignedProjects[$user['login']]) > 0 && EfrontUser::isOptionVisible('projects')) {
