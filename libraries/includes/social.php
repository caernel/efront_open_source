	...
	if ($_GET['op'] == "dashboard") {	
		...
		if (isset($forum_lessons_ID[0]['id'])) {																   //If there is a forum category associated to this lesson (and the user is eligible to use it), display corresponding links
			...
			$forum_options = array(
					...
					array('text' => _SENDMESSAGEATFORUM, 'image' => "16x16/add.png", 'href' => basename($_SERVER['PHP_SELF'])."?ctg=forum&add=1&type=topic&category=".$forum_lessons_ID[0]['id'], 'onClick' => "eF_js_showDivPopup(event, '"._NEWMESSAGE."', new Array('650px', '450px'));", 'target' => 'POPUP_FRAME')
					);
		...
		if (!isset($currentUser -> coreAccess['calendar']) || $currentUser -> coreAccess['calendar'] != 'hidden') {
			...
			if (!isset($currentUser -> coreAccess['calendar']) || $currentUser -> coreAccess['calendar'] == 'change') {
				$calendarOptions[] = array('text' => _ADDCALENDAR,  'image' => "16x16/add.png", 'href' => basename($_SERVER['PHP_SELF'])."?ctg=calendar&add=1&view_calendar=".$view_calendar."&popup=1", "onClick" => "eF_js_showDivPopup(event, '"._ADDCALENDAR."', 2)", "target" => "POPUP_FRAME");
			}
		...
		if (EfrontUser::isOptionVisible('func_comments')) {
			$my_info_options = array(array('text' => _ADDCOMMENTTOMYPROFILE, 'image' => "16x16/edit.png", 'href' => $_SESSION['s_type'].".php?ctg=social&op=comments&action=insert&popup=1&user=". $currentUser -> user['login'], 'onClick' => "eF_js_showDivPopup(event, '"._USERPROFILE."', 1)", 'target' => 'POPUP_FRAME'));
